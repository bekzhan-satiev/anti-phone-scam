...
fun StatisticScreen(callViewModel: CallViewModel = hiltViewModel()) {
    ...
    val pieData = listOf(
        Pie(label = stringResource(R.string.safe), data = statusCounts[CallStatus.SAFE]?.toDouble() ?: 0.0, color = Color.Green),
        Pie(label = stringResource(R.string.suspicious), data = statusCounts[CallStatus.SUSPICIOUS]?.toDouble() ?: 0.0, color = Color.Yellow),
        Pie(label = stringResource(R.string.blocked), data = statusCounts[CallStatus.BLOCKED]?.toDouble() ?: 0.0, color = Color.Red),
    )
